...
#number of citizens over 65 years old in each prefecture
prefecture_gp <- read_xlsx("data/prefecture_population.xlsx") 
...
joint_data_final <- joint_data |> 
  left_join(prefecture_gp_sum, join_by(NAME_1 == prefecture_new)) 

healthcare_facilities_per_city <- count(as_tibble(joint_data_final), NAME_2) 

distinct_healthcare_facilities_per_city <- count(as_tibble(joint_data_final), NAME_2, amenity)
...
## EDA
```{r}
#distribution of the density of healthcare facilities using a histogram (getting the range of values)
new_data |> 
...
#distribution of the geriatric pop
new_data |> 
  ggplot(aes(x = year_2020_percentage)) +
  geom_histogram(binwidth = 3) 

new_data |> 
  ggplot(aes(y = year_2020_percentage)) +
  geom_boxplot() 

#correlation between geriatric pop and the number of healthcare facilities
cor(new_data$year_2020_percentage, 
    new_data$n, 
    use = "complete.obs",
    method = "pearson")

var(new_data$year_2020_percentage, 
    new_data$n, 
    na.rm = TRUE)

#scatterplot of number vs pop percenntage
new_data |> 
  ggplot(aes(x = year_2020_percentage, y = n)) +
  geom_point()
```
...
learnr::run_tutorial("quiz-4", package = "r4dsQuizzes")
